...
type RenderProcessGoneDetails = Electron.RenderProcessGoneDetails & { exitCode?: number };
...
export function onRendererProcessGone(
  ...
  if (supportsRenderProcessGone) {
    app.on('render-process-gone', (_, contents, details) => {
      logger.log('got render process gone event', details.reason);
...
export function onChildProcessGone(reasons: Readonly<ExitReason[]>, callback: (details: Details) => void): void {
  if (version.major >= 11) {
    app.on('child-process-gone', (_, details) => {
      logger.log('got child process gone event', details.name, details.type, details.reason);
      if (reasons.includes(details.reason)) {
        callback(details);
      }
    });
